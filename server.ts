import { createClient, type InArgs, type ResultSet, type Transaction } from "@libsql/client";
import { join } from "node:path";

const url = process.env.TURSO_DATABASE_URL;
const authToken = process.env.TURSO_AUTH_TOKEN;
if (!url) {
  console.error("Не задан TURSO_DATABASE_URL. Скопируйте .env.example в .env и заполните подключение к Turso.");
  process.exit(1);
}
...
console.log(`База: Turso ${new URL(url).host}`);
